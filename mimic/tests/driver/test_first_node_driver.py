...
from mimic.engine.driver import first_node_driver
from mimic.tests.db import base
from mimic.tests.driver.utils import FakeDBAPI


class FirstNodeDriverTestCase(base.DbTestCase):

    def setUp(self):
        super(FirstNodeDriverTestCase, self).setUp()

    def test_action(self):
        with mock.patch("mimic.db.api.get_instance",
                        mock.MagicMock(return_value=FakeDBAPI())):
                        self.dbapi = dbapi.get_instance()
                        driver = first_node_driver.get_backend("FirstNode",
                                        "env=master",
                                        ["sunfire::masternode"],
                                        ["master"])
